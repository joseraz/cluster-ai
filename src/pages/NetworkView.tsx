...
import { ReactFlow, addEdge, Controls, useNodesState, useEdgesState, Node, Edge } from '@xyflow/react';
...
// Review contacts (orange)
{
  ...
  data: {
    label: 'Steve Anderson',
    name: 'Steve Anderson',
    company: 'Business Dev',
    role: 'Partner',
    category: 'review'
  },
  style: {
...
const NetworkView = () => {
  ...
  const filteredNodes = nodes.filter(node => (node.data.name as string).toLowerCase().includes(searchTerm.toLowerCase()) || (node.data.company as string)?.toLowerCase().includes(searchTerm.toLowerCase()));
  const categories = [{
    name: 'All',
    count: 35,
    color: '#0077B5',
  ...
  return <div className="h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 relative">
        ...
        <ReactFlow nodes={searchTerm ? filteredNodes : nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} fitView style={{
        ...
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2">
          <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-2 flex items-center gap-2 min-w-96">
            ...
            <Input placeholder="User | name (starts with): Will | WROTE | Review | (any) | Business | (any) | Category" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="border-0 focus-visible:ring-0 bg-transparent flex-1" />
